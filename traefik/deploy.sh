...
# We use sudo here just in case the user was just added to the docker group 
# and hasn't logged out/in yet to apply the group permissions.
...
if [ -f "docker-compose.yml" ]; then
    ...
    echo "Error: docker-compose.yml not found in the current directory. Please run this script from the extracted GitHub repository folder."
    exit 1
fi
...
if [ -f "middlewares.yml" ]; then
    ...
    echo "Warning: middlewares.yml not found in the current directory. Global security headers will not be applied."
fi
...
# 7. Traefik Dashboard Authentication Secret
echo ""
...
read -p "Enter the Base Domain (e.g., app.ssnl.bz): " INPUT_DOMAIN
read -p "Enter the Let's Encrypt Email (e.g., admin@ssnl.bz): " INPUT_EMAIL

export BASE_DOMAIN=${INPUT_DOMAIN:-app.ssnl.bz}
export LE_EMAIL=${INPUT_EMAIL:-admin@ssnl.bz}

# 10. Deploy to Swarm
echo ""
echo "--> Deploying Proxy Stack to the Swarm..."
# Export variables directly into the docker stack deploy command environment
sudo -E env "BASE_DOMAIN=$BASE_DOMAIN" "LE_EMAIL=$LE_EMAIL" docker stack deploy -c /opt/proxy/docker-compose.yml proxy
